...
- 新增全局音乐能力：应用初始化后按持久化偏好自动播放背景音乐，支持上一首 / 下一首 / 播放暂停 / 静音切换，并支持 4 类短音效：专注开始、专注结束、按钮打开、按钮返回。
- 新增 UI 音效防重能力：针对单次交互链路内的重复触发，增加 controller 级别短窗口去重与 audio service 级别并发/冷却保护，避免按钮音效偶发双响。
- 新增本地持久化恢复：`pomodoro`、`xp`、`music`、`supervisor` 四类状态均通过 `SharedPreferences` 恢复。
- 新增面向 controller 的测试：覆盖后台监管、XP 结算、音乐自动播放 / 生命周期恢复与音效触发等关键链路。
...
- `global-background-music-and-phase-sfx`: 全局背景音乐自动播放、用户覆盖持久化、阶段与按钮音效、生命周期暂停 / 恢复。
